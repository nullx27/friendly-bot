...
var NotifyHandler_1 = require("./handlers/NotifyHandler");
...
var FriendlyBot = /** @class */ (function () {
    function FriendlyBot(container) {
        ...
        this.container.register('logger', Logger_1.logger);
        this.token = container.get('config');
    ...
    FriendlyBot.prototype.registerEventHandlers = function () {
        var _this = this;
        this.client.once('ready', function () { return _this.readyEvent.bind(_this); });
        this.client.on('disconnect', function (event) {
            _this.logger.error("Disconnected with close event: " + event.code);
        });
        this.client.on('error', function (error) { return _this.logger.error(error); });
        this.client.on('warn', function (warning) { return _this.logger.warning(warning); });
